...
const TOP_USERS = [
  { rank: 1, name: 'Carlos Mendoza', area: 'Ingeniería', score: 2450, avatar: '' },
  { rank: 2, name: 'Ana Sofía Ruiz', area: 'Marketing', score: 2320, avatar: '' },
  { rank: 3, name: 'Roberto Garza', area: 'Operaciones', score: 2180, avatar: '' },
  { rank: 4, name: 'Elena Torres', area: 'Finanzas', score: 1950, avatar: '' },
  { rank: 5, name: 'Miguel Ángel', area: 'Logística', score: 1870, avatar: '' },
];

const TOP_AREAS = [
  { area: 'Ingeniería', engagement: 94, participants: 1250 },
  { area: 'Operaciones', engagement: 88, participants: 1100 },
  { area: 'Ventas', engagement: 82, participants: 980 },
  { area: 'Marketing', engagement: 79, participants: 850 },
  { area: 'Finanzas', engagement: 75, participants: 620 },
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-12 flex flex-col md:flex-row items-end justify-center gap-4 sm:gap-8 py-12">
          ...
          >
            <div className="relative mb-4">
              <img src={TOP_USERS[1].avatar} alt="" className="w-20 h-20 rounded-full border-4 border-slate-200 shadow-xl" />
            ...
            <div className="bg-white p-6 rounded-t-3xl border border-slate-100 shadow-sm w-48 text-center h-32 flex flex-col justify-center">
              <p className="font-bold text-on-surface truncate">{TOP_USERS[1].name}</p>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{TOP_USERS[1].area}</p>
              <p className="text-xl font-black text-primary mt-2">{TOP_USERS[1].score} XP</p>
            </div>
          </motion.div>
          ...
          >
            <div className="relative mb-4">
              ...
              <img src={TOP_USERS[0].avatar} alt="" className="w-24 h-24 rounded-full border-4 border-amber-400 shadow-2xl" />
            ...
            <div className="bg-white p-8 rounded-t-[2.5rem] border border-amber-100 shadow-xl w-56 text-center h-44 flex flex-col justify-center">
              <p className="font-black text-on-surface truncate text-lg">{TOP_USERS[0].name}</p>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{TOP_USERS[0].area}</p>
              <p className="text-2xl font-black text-primary mt-2">{TOP_USERS[0].score} XP</p>
            </div>
          </motion.div>
          ...
          >
            <div className="relative mb-4">
              <img src={TOP_USERS[2].avatar} alt="" className="w-20 h-20 rounded-full border-4 border-orange-200 shadow-xl" />
            ...
            <div className="bg-white p-6 rounded-t-3xl border border-slate-100 shadow-sm w-48 text-center h-28 flex flex-col justify-center">
              <p className="font-bold text-on-surface truncate">{TOP_USERS[2].name}</p>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{TOP_USERS[2].area}</p>
              <p className="text-xl font-black text-primary mt-2">{TOP_USERS[2].score} XP</p>
            </div>
          </motion.div>
        </div>
            ...
            {TOP_USERS.map((user, i) => (
              <div key={i} className="px-8 py-5 flex items-center gap-6 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{user.area}</p>
                </div>
                <p className="font-black text-primary">{user.score} XP</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-primary rounded-[2.5rem] p-8 text-white shadow-xl shadow-primary/20">
            <h3 className="text-xl font-bold mb-8">Top 5 Áreas</h3>
            <div className="space-y-6">
              {TOP_AREAS.map((area, i) => (
                <div key={i} className="space-y-2">
                  <div className="flex justify-between items-end">
                    <p className="text-sm font-bold">{area.area}</p>
                    <p className="text-[10px] font-black uppercase tracking-widest text-white/60">{area.participants} Participantes</p>
                  </div>
