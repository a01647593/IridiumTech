...
import { MOCK_ADMIN_STATS, MOCK_METRICS } from '../constants';
import { 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Usuarios Totales', value: MOCK_ADMIN_STATS.totalUsers, icon: 'group', color: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Usuarios Activos', value: MOCK_ADMIN_STATS.activeUsers, icon: 'bolt', color: 'text-amber-600', bg: 'bg-amber-50' },
          { label: 'Tasa de Finalización', value: `${MOCK_ADMIN_STATS.completionRate}%`, icon: 'task_alt', color: 'text-green-600', bg: 'bg-green-50' },
          { label: 'Score Promedio', value: MOCK_ADMIN_STATS.avgScore, icon: 'analytics', color: 'text-purple-600', bg: 'bg-purple-50' },
        ].map((stat, i) => (
          <motion.div 
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <h3 className="text-lg font-bold text-on-surface mb-8">Tendencia de Adopción (6 meses)</h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={MOCK_ADMIN_STATS.adoptionTrend}>
                <defs>
        ...
        <div className="lg:col-span-4 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={MOCK_ADMIN_STATS.usersByArea}
                  cx="50%"
                  cy="50%"
                  innerRadius={60}
                  outerRadius={80}
                  ...
                  dataKey="value"
                >
                  {MOCK_ADMIN_STATS.usersByArea.map((entry, index) => (
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {MOCK_ADMIN_STATS.usersByArea.map((area, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50">
                ...
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Abandono</th>
              ...
              {MOCK_METRICS.map((metric, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  <td className="px-8 py-5">
                    <p className="font-bold text-on-surface">{metric.area}</p>
                  </td>
                  <td className="px-8 py-5">
                    <p className="text-sm text-slate-500 font-medium">{metric.count}</p>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5">
                    <p className="text-sm text-red-500 font-bold">{metric.abandonmentRate}%</p>
                  </td>
                  <td className="px-8 py-5">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      metric.engagement > 85 ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'
                    }`}>
                      {metric.engagement > 85 ? 'Excelente' : 'En Mejora'}
                    </span>
                  </td>
                </tr>
              ))}
